...
def preprocess_image(image_path):
	...
	model.load_weights('C:/final-project-aug/weights/vgg_face_weights.h5')
...
if exists != True: #initializations lasts almost 1 hour. but it can be run once.
	...
	mat = scipy.io.loadmat('../dataset/wiki.mat')
	print("wiki.mat meta data file loaded")
	...
	def extractNames(name):
		...
		return cv2.imread("../database/%s" % image_path[0]) #pixel values in scale of 0-255
	...
	for (x,y,w,h) in faces:
		if w > 0:
			...
			if similarity > 50:
				...
				celebrity_img = cv2.imread("../database/%s" % full_path)
